...
public partial class GameConfigWindow : AdonisWindow
{
    ...
    public GameConfigWindow(GameConfigWindowViewModel viewModel, GameContainer gameContainer, AsyncFileWatcher watcher,
        DllUpdater updater, Logger logger, GameInfo? gameInfo)
    {
        ViewModel = viewModel;
        _logger = logger;
        _newGame = true;
        ViewModel.GameInfo = new GameInfo("", "", LibraryType.Manual);
        if (gameInfo is not null)
        {
            watcher.RemoveFile(gameInfo);
            ViewModel.GameInfo = new GameInfo(gameInfo);
    ...
    private async void btnPath_Click(object sender, RoutedEventArgs e)
    {
        ...
        if (dlg.ShowDialog() == true)
        {
            ...
            var (_, ex) = await ViewModel.GameInfo.GatherInstalledVersions();
            if (ex is UnauthorizedAccessException)
            {
                ...
                var messageBox = new MessageBoxModel
                {
                    ...
                    Text =
                        "You don't have enough rights to access the selected path. Try running Dlss Updater as admin.",
